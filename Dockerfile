...
ARG HYKU_PATH
COPY --chown=1001:101 $HYKU_PATH/Gemfile* /app/samvera/hyrax-webapp/
RUN git config --global --add safe.directory /app/samvera && \
  bundle install --jobs "$(nproc)"

COPY --chown=1001:101 $HYKU_PATH/bin/db-migrate-seed.sh /app/samvera/
# This is specifically NOT $HYKU_PATH but the parent directory
